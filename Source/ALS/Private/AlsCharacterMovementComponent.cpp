	...
	while ( (remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) && CharacterOwner && (CharacterOwner->Controller || bRunPhysicsWithNoController || HasAnimRootMotion() || CurrentRootMotion.HasOverrideVelocity() || (CharacterOwner->GetLocalRole() == ROLE_SimulatedProxy)) )
	{
		...
		else
		{
			...
			if ( IsFalling() )
			{
				...
				const auto DesiredDist = UE_REAL_TO_FLOAT(Delta.Size());
				if (DesiredDist > UE_KINDA_SMALL_NUMBER)
				{
					const auto ActualDist = UE_REAL_TO_FLOAT((UpdatedComponent->GetComponentLocation() - OldLocation).Size2D());
	...
	if (DownwardSweepResult != NULL && DownwardSweepResult->IsValidBlockingHit())
	{
		...
		if (bIsDownward && bIsVertical)
		{
			...
			if (IsWithinEdgeTolerance(DownwardSweepResult->Location, DownwardSweepResult->ImpactPoint, PawnRadius))
			{
				...
				const auto FloorDist = UE_REAL_TO_FLOAT(RotateWorldToGravity(CapsuleLocation - DownwardSweepResult->Location).Z);
